...
# ── Step 2: Resolve NVIDIA API key (BEFORE onboard) ──────────────
info "Resolving NVIDIA API key..."

if [ -n "${NVIDIA_API_KEY:-}" ]; then
  ok "NVIDIA_API_KEY already in environment"
else
  if [ -f "$SCRIPT_DIR/.env" ]; then
    NVIDIA_API_KEY=$(python3 -c "
import re
for line in open('$SCRIPT_DIR/.env'):
    m = re.match(r'^NVIDIA_API_KEY=[\"' + \"'\" + r']?([^\"' + \"'\" + r' \n]+)', line.strip())
    if m: print(m.group(1)); break
" 2>/dev/null || true)
    [ -n "${NVIDIA_API_KEY:-}" ] && ok "NVIDIA_API_KEY loaded from $SCRIPT_DIR/.env"
  fi
...
" 2>/dev/null || true)
  ...
  if [ -z "${NVIDIA_API_KEY:-}" ]; then
    echo ""
    echo -e "  ${YELLOW}Get your free API key at: https://build.nvidia.com${NC}"
    echo ""
    echo -n "  NVIDIA API Key (nvapi-...): "
    read -r NVIDIA_API_KEY
    [ -z "${NVIDIA_API_KEY:-}" ] && fail "NVIDIA API key is required."
    echo "NVIDIA_API_KEY=\"$NVIDIA_API_KEY\"" > "$SCRIPT_DIR/.env"
    ok "API key saved to $SCRIPT_DIR/.env"
  fi
fi
...
if [ "${LIVE_COUNT:-0}" -eq 0 ]; then
  ...
  echo -e "  ${YELLOW}You will be prompted to choose your inference provider, model, and sandbox name.${NC}"
  echo ""
